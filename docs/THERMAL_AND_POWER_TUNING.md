...
| **Balanced 24/7 (Default)** | `40 W` | `35 W` | `85 °C` | `~2.85 – 3.00 GHz` | Moderate fan, ~75–82°C | **Continuous 24/7 compute (BOINC)** |
| **Max Turbo / Cooler Mod** | `45 W` | `45 W` | `95 °C` | `~3.02 – 3.05 GHz` | Maximum fan, ~85–88°C | Short benchmarks, external cooling mods |
...
def apply_fast_ppt():
    ...
    try:
        subprocess.run([
            RYZENADJ_BIN,
            "--stapm-limit=35000",        # Sustained power limit (in mW)
            "--fast-limit=40000",         # Short burst power limit (in mW)
            "--slow-limit=35000",         # Sustained package power limit (in mW)
            "--vrm-current=50000",        # TDC limit (in mA)
            "--vrmmax-current=65000",     # EDC limit (in mA)
            "--vrmsoc-current=14000",     # SoC TDC limit (in mA)
            "--vrmsocmax-current=18000",  # SoC EDC limit (in mA)
            "--tctl-temp=85",             # Temperature cap (in °C)
